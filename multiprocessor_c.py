...
def init_united(united_df):
    """Initializer for worker processes to set a global read-only DataFrame."""
...
def get_flight_phase(trajectory_df):
    if trajectory_df.empty:
        return []
    ts = (trajectory_df['timestamp'] - trajectory_df['timestamp'].iloc[0]).dt.total_seconds()  # seconds
    alt = trajectory_df['altitude'].values  # feet
    spd = trajectory_df['groundspeed'].values  # knots
    roc = trajectory_df['vertical_rate'].values  # feet per minute
...
def extract_phases(trajectory_df, min_length, min_period):
    if trajectory_df.empty:
        return pd.DataFrame(), pd.DataFrame()  # Always return two DataFrames
    ...
    # Always return a tuple of two DataFrames
    return result.get('CL', pd.DataFrame()), result.get('DE', pd.DataFrame())

def process_flight(flight_id, min_length, min_period):
    global _united_global
    rows = []
    ...
    for _, seg in segments.iterrows():
        ...
        climb_phase_df, descend_phase_df = extract_phases(filtered, min_length, min_period)
...
def run_multiprocess_united(united_df, num_workers=10, min_length=10, min_period=5):
    unique_ids = united_df['flight_id'].unique()
    with Pool(num_workers, initializer=init_united, initargs=(united_df,)) as pool:
        results = pool.map(partial(process_flight, min_length=min_length, min_period=min_period), unique_ids)
